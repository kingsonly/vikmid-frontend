...
import { MessageSquare, BookOpen, ShoppingBag, Calendar, CreditCard, Mail, LinkIcon, Users, Settings, Menu, X, Store } from 'lucide-react'
...
export default function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen bg-gray-900 text-gray-100">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-gray-800/80 backdrop-blur-xl p-6 flex flex-col transition-transform duration-300 ease-in-out transform border-r border-gray-700 ${
        ...
        <nav className="flex-1">
          <ul className="space-y-2">
            {viewMode === "creator" ? (
              <>
                <li>
                  <Link href="/dashboard" className="flex items-center p-2 hover:text-indigo-400 transition-colors">
                    <LinkIcon className="mr-2" /> Dashboard
                  </Link>
                </li>
        ...
        <div className="mt-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                id="view-mode-creator"
                checked={viewMode === "creator"}
                onCheckedChange={toggleViewMode}
              />
              <Label htmlFor="view-mode-creator">
                {viewMode === "creator" ? "Creator" : "Fan"}
              </Label>
            </div>
          </div>
          <Link
